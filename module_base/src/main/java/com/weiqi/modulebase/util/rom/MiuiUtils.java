...
//            }
    ...
    public static void applyMiuiPermission(Context context) {
        ...
        Intent intent = null;
        if (versionCode == 5) {
            intent = goToMiuiPermissionActivity_V5(context);
        } else if (versionCode == 6) {
            intent = goToMiuiPermissionActivity_V6(context);
        } else if (versionCode == 7) {
            intent = goToMiuiPermissionActivity_V7(context);
        } else if (versionCode == 8) {
            intent = goToMiuiPermissionActivity_V8(context);
        } else {
    ...
    public static Intent getMuUiPermissionActivity(Context context) {
        int versionCode = getMiuiVersion();
        Intent intent = null;
        if (versionCode == 5) {
            intent = goToMiuiPermissionActivity_V5(context);
...
//        }
     ...
     * 小米 V6 版本 ROM权限申请
     */
    public static Intent goToMiuiPermissionActivity_V6(Context context) {
        Intent intent = new Intent("miui.intent.action.APP_PERM_EDITOR");
...
//            FloatWindowManager.getInstance().startActivityOrForResult(context, intent);
     ...
     * 小米 V7 版本 ROM权限申请
     */
    public static Intent goToMiuiPermissionActivity_V7(Context context) {
        Intent intent = new Intent("miui.intent.action.APP_PERM_EDITOR");
        intent.setClassName("com.miui.securitycenter", "com.miui.permcenter.permissions.AppPermissionsEditorActivity");
        intent.putExtra("extra_pkgname", context.getPackageName());
